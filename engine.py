...
def _convert_timing_format(timing):
    ...
    # we automatically wait for crank to finish rotation before coming back to first cylinder
    timing[0] = 0
...
class Engine:
    ...
    def _gen_audio_one_engine_cycle(self):
        ...
        between_fire_duration = sec_between_fires / self.strokes * \
            (self.strokes-1)  # when exhaust valve is closed
        ...
        for cylinder in range(0, self.cylinders):
            unequalms = (
                # unequal converted from milliseconds to seconds
                self.unequal[cylinder]/1000
            ...
            # + unequalms # 180 degrees crankshaft rotation per stroke
            before_fire_duration = (
                self.timing[cylinder] / 180) / strokes_per_sec
            before_fire_snd = audio_tools.slice(
                self.between_fire_snd, before_fire_duration+unequalms)
            after_fire_duration = between_fire_duration - before_fire_duration  # - unequalms
            after_fire_snd = audio_tools.slice(
                self.between_fire_snd, after_fire_duration)
            ...
            if unequalms:  # if unequal parameter set for cylinder
                ...
                before_fire_snd = audio_tools.slice(
                    ...
                    # - self.previousms  # current duration plus difference between unequals
                    before_fire_duration + unequalms
                )  # make interval before sound smaller
                bufsunequal.append(
                    audio_tools.concat(
                        # combustion + silence
                        [before_fire_snd, fire_snd, after_fire_snd]
                    )
        ...
        bufs = [audio_tools.pad_with_zeros(
            buf, max_buf_len-len(buf)) for buf in bufs]
        ...
        bufsunequal = [audio_tools.pad_with_zeros(
            buf, max_buf_len_unequal-len(buf)) for buf in bufsunequal]
        ...
        if sum(engine_snd_unequal) > 0:  # if unequal buffer contains anything
            if cfg.sound_merge_method == "average":  # average of both buffers
                engine_snd = np.mean(
                    [engine_snd, engine_snd_unequal[:len(engine_snd)]], axis=0)
            elif cfg.sound_merge_method == "max":  # maximum values of both buffers
                engine_snd = np.maximum(
                    engine_snd, engine_snd_unequal[:len(engine_snd)])
    ...
    def gen_audio(self, num_samples):
        ...
        while len(self._audio_buffer) + len(engine_snd) < num_samples:
            # this is unlikely to run more than once
            engine_snd = audio_tools.concat([engine_snd, engine_snd])
    ...
    def throttle(self, fraction):
        ...
        if fraction > 0:
            if self._rpm < self.limiter_rpm:
                ...
                self._rpm += (fraction / 4)  # self.strokes)
            else:
